...
# possible values for the mean, cf ui.R mx0, mx
mu.vec<-c(25:75)
...
shinyServer(function(input, output){
  ...
  output$plotEmp <- renderPlot({
      ...
      if(v$relPl){
          ...
          lines(x<-c(v$mx,v$mx),y <- c(0,cv$maxdmx*1.8),lty=1,lwd=3,col=color.blue)
          text(v$mx,cv$maxdmx*1.95,labels=bquote(mu),cex=3.5,col=color.blue)
      }
      ...
      ##-------------------------------------------              
         ...
         if(v$rejPl == "rejPloui"){
             if(v$relPl == FALSE){
                 ...
                 text(v$mx0,cv$maxdmx*1.95,labels=bquote(mu[.0]),cex=2.5,col=oui.color.true)
                 help.color.vec<-cv$ic.k.inc.mu.color.vec.toshow
             }
             if(v$relPl == TRUE && v$mx0 == v$mx){
                 ## Plot mean mx1 
                 lines(x<-c(v$mx,v$mx),y <- c(0,cv$maxdmx*1.8),lty=1,lwd=3,col=color.true)
                 text(v$mx,cv$maxdmx*1.95,labels=bquote(paste(mu[0],'=',mu)),cex=3.5,col=color.true)
             ...
             if(v$relPl == TRUE && v$mx0 != v$mx){
                # lines(x<-c(v$mx,v$mx),y <- c(0,cv$maxdmx*1.8),lty=1,lwd=3,col=oui.color.true)
                 ...
                 lines(x<-c(v$mx,v$mx),y <- c(0,cv$maxdmx*1.8),lty=1,lwd=3,col=color.blue)
                # text(v$mx,cv$maxdmx*1.95,labels=bquote(mu),cex=3.5,col=color.false)
                 text(v$mx0,cv$maxdmx*1.95,labels=bquote(mu[0]),cex=3.5,col=color.false)
                 text(v$mx,cv$maxdmx*1.95,labels=bquote(mu),cex=3.5,col=color.blue)
         ...
         if(v$rejPl == "rejPloui"){
             mtext(bquote(paste("Décision : ", N == .(cv$n.samples), sep="")),side=4,line=1,at=signif(cv$maxdmx,1)*(-0.1),las=2)
         } else {
             mtext(bquote(paste("Intervalle : ", N == .(cv$n.samples), sep="")),side=4,line=1,at=signif(cv$maxdmx,1)*(-0.1),las=2)
         }
         if(cv$samples.x.n.toshow>0){
             for(i in 1:cv$samples.x.n.toshow){
                 if(v$rejPl == "rejPloui"){
                 ...
                 if(v$thresholds == "formula"){
                     if(v$CVk == 'eCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(mu[0] %in% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-c,bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(mu[0] %notin% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-c,bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3) 
                             }
                         } else {
                     ...
                     if(v$CVk == 'vCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(mu[0] %in% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-c*'*'*sigma/sqrt(n),bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c*'*'*sigma/sqrt(n)),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(mu[0]%notin% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-c*'*'*sigma/sqrt(n),bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c*'*'*sigma/sqrt(n)),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             }
                         } else {
                     ...
                     if(v$CVk == 'sCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(mu[0] %in% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-c*'*'*s[.(cv$samples.x.i.vec.toshow[i])]/sqrt(n),bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c*'*'*s[.(cv$samples.x.i.vec.toshow[i])]/sqrt(n)),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(mu[0] %notin% group("[",list(bar(x)[.(cv$samples.x.i.vec.toshow[i])]-.(v$k)*'*'*s[.(cv$samples.x.i.vec.toshow[i])]/sqrt(n),bar(x)[.(cv$samples.x.i.vec.toshow[i])]+c*'*'*s[.(cv$samples.x.i.vec.toshow[i])]/sqrt(n)),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             }
                         } else {
                 ...
                 if(v$thresholds == "calcul"){
                     if(v$CVk == 'eCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(.(v$mx0) %in% group("[",list(.(sprintf("%.2f",cv$samples.x.m.vec.toshow[i]))-.(v$k),.(sprintf("%.2f",cv$samples.x.m.vec.toshow[i]))+.(v$k)),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(.(v$mx0) %notin% group("[",list(.(sprintf("%.2f",cv$samples.x.m.vec.toshow[i]))-.(v$k),.(sprintf("%.2f",cv$samples.x.m.vec.toshow[i]))+.(v$k)),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             }
                         } else {
                     ...
                     if(v$CVk == 'vCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(.(v$mx0) %in% group("[",list(.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))-.(v$k)*'*'*.(v$sx)/.(sprintf("%.1f",v$n^.5)),.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))+.(v$k)*'*'*.(v$sx)/.(sprintf("%.1f",v$n^.5))),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(.(v$mx0) %notin% group("[",list(.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))-.(v$k)*'*'*.(v$sx)/.(sprintf("%.1f",v$n^.5)),.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))+.(v$k)*'*'*.(v$sx)/.(sprintf("%.1f",v$n^.5))),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             }
                         } else {
                     ...
                     if(v$CVk == 'sCVk'){
                         if(v$rejPl == "rejPloui"){
                             if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                                 mtext(bquote(paste(.(v$mx0) %in% group("[",list(.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))-.(v$k)*'*'*.(sprintf("%.1f",cv$samples.x.sd.vec.toshow[i]))/.(sprintf("%.1f",v$n^.5)),.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))+.(v$k)*'*'*.(sprintf("%.1f",cv$samples.x.sd.vec.toshow[i]))/.(sprintf("%.1f",v$n^.5))),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             } else {
                                 mtext(bquote(paste(.(v$mx0) %notin% group("[",list(.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))-.(v$k)*'*'*.(sprintf("%.1f",cv$samples.x.sd.vec.toshow[i]))/.(sprintf("%.1f",v$n^.5)),.(sprintf("%.1f",cv$samples.x.m.vec.toshow[i]))+.(v$k)*'*'*.(sprintf("%.1f",cv$samples.x.sd.vec.toshow[i]))/.(sprintf("%.1f",v$n^.5))),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                             }
                         } else {
                 ...
                 if(v$thresholds == "result"){
                     if(v$rejPl == "rejPloui"){
                         if(cv$ic.k.inc.allmu.mat.toshow[i,v$mx0-mu.vec[1]+1]){
                             mtext(bquote(paste(.(v$mx0) %in% group("[",list(.(sprintf("%.2f",cv$ic.k.limit.mat.toshow[i,1])),.(sprintf("%.2f",cv$ic.k.limit.mat.toshow[i,2]))),"]") %=>% NRH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                         } else {
                             mtext(bquote(paste(.(v$mx0) %notin% group("[",list(.(sprintf("%.2f",cv$ic.k.limit.mat.toshow[i,1])),.(sprintf("%.2f",cv$ic.k.limit.mat.toshow[i,2]))),"]") %=>% RH[0],sep="")),side=4,line=1,at=cv$samples.y.mat.toshow[i,1],las=2,col=help.color.vec[i],cex=1.3)
                         }
                     } else {                                      
          ...
          if(v$rejPl == "rejPloui" && v$freqPl == "freqPloui"){
              par(mai=c(0.5,0.7,.5,0.3))
              ...
              barplot.kH1<-barplot(includes,names.arg=mu.vec,xlab=xlab.text,ylab="%",ylim=c(0,150),yaxp=c(0,100,2),col=c(non.color.false,non.color.true),cex.names=1.25,cex.lab=2.3,cex.axis=1.2,beside=FALSE)
                                        #  barplot.kH1 is the vector of positions of th bars which we use next
              if(v$relPl == FALSE && cv$n.samples>0){
                  barplot.spp<-barplot(matrix(c(100-cv$pc.ic.k.inc.allmu.vec[(v$mx0-mu.vec[1]+1)],cv$pc.ic.k.inc.allmu.vec[(v$mx0-mu.vec[1]+1)]),ncol=1),col=c(oui.color.false,oui.color.true), add=TRUE,beside=FALSE,space=(barplot.kH1[(v$mx0-mu.vec[1]+1)]-0.5),axes=FALSE)
                  ...
                  mtext(bquote(paste("% de couverture de ",mu[0],"=",.(v$mx0)," par les intervalle de confiance",sep=" ")),side=3,line=-3,adj = 0)
                  mtext(bquote(paste("n",sep=" ")),side=3,line=-5,adj = 1,at=13)
                  mtext(bquote(paste("%",sep=" ")),side=3,line=-5,adj = 1,at=16)
                  mtext(bquote(paste("IC contient ",mu[0]," : ",sep=" ")),side=3,line=-7,adj = 1,at=10,col=text.color.true)
                  ...
                  mtext(bquote(paste("IC ne contient pas ",mu[0]," : ",sep=" ")),side=3,line=-9,adj = 1,at=10,col=text.color.false)
              ...
              if( v$relPl == TRUE  && v$mx0 == v$mx && cv$n.samples>0){
                  barplot.spp<-barplot(matrix(c(100-cv$pc.ic.k.mu0.inc.allmu.vec[(v$mx-mu.vec[1]+1)],cv$pc.ic.k.mu0.inc.allmu.vec[(v$mx-mu.vec[1]+1)]),ncol=1),col=c(color.false,color.true), add=TRUE,beside=FALSE,space=(barplot.kH1[(v$mx0-mu.vec[1]+1)]-0.5),axes=FALSE)
                  ...
                  mtext(bquote(paste("Test d'hypothèses ")),side=3,line=-1,adj = 0)
                  mtext(bquote(paste(H[0]," : ",mu,"=",.(v$mx0)," vs ",H[1]," : ",mu,"≠",.(v$mx0),sep=" ")),side=3,line=-1,adj = 0,at=10,col=color.true)
                  mtext(bquote(paste("lorsque ",mu,"=",.(v$mx)," est la moyenne de la population d'origine:",sep=" ")),side=3,line=-3,adj = 0,col="blue")

                  mtext(bquote(paste("n",sep=" ")),side=3,line=-5,adj = 1,at=13)
                  mtext(bquote(paste("%",sep=" ")),side=3,line=-5,adj = 1,at=16)
                  mtext(bquote(paste("Non rejet de ",H[0]," : ",sep=" ")),side=3,line=-7,adj = 1,at=10,col=color.true)
                  ...
                  mtext(bquote(paste("Rejet de ",H[0]," : ",sep=" ")),side=3,line=-9,adj = 1,at=10,col=color.false)
              ...
              if(v$relPl == TRUE && v$mx0 != v$mx && cv$n.samples>0){
                  ...
                  mtext(bquote(paste("Test d'hypothèses ")),side=3,line=-1,adj = 0)
                  mtext(bquote(paste(H[0]," : ",mu,"=",.(v$mx0)," vs ",H[1]," : ",mu,"≠",.(v$mx0),sep=" ")),side=3,line=-1,adj = 0,at=10,col=color.false)
                  mtext(bquote(paste("lorsque ",mu,"=",.(v$mx)," est la moyenne de la population d'origine:",sep=" ")),side=3,line=-3,adj = 0,col="blue")

                  mtext(bquote(paste("n",sep=" ")),side=3,line=-5,adj = 1,at=13)
                  mtext(bquote(paste("%",sep=" ")),side=3,line=-5,adj = 1,at=16)
                  mtext(bquote(paste("Non rejet de ",H[0]," : ",sep=" ")),side=3,line=-7,adj = 1,at=10,col=color.false)
                  ...
                  mtext(bquote(paste("Rejet de ",H[0]," : ",sep=" ")),side=3,line=-9,adj = 1,at=10,col=color.true)
