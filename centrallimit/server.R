...
shinyServer(function(input, output){ 
  ...
  getComputedValues<-reactive({
    ...
    if(cv$n.samples>0){
      for(i in 1:cv$n.samples){
        ...
        if (input$dist == "DE"){cv$samples.x[[i]]<-round(rv$samples.z[[i]]*(1/v$Lambda),2)}
 ...
 #####Output 1 : DISTRIBUTION THEORIQUE #####
 ...
 if (input$dist == "DB"){
   ...
   dens <- getY()
   plot(dens,xlab="",bty="n",main=HTML("Distribution théorique"),type="l", xlim = c(x.lim.inf, x.lim.sup), cex.main = cex.main.title)
 }
 
 else {
  Y<-getY()
  par(mai=c(0.5,0.5,0.5,0.5))
  plot(X,Y,xlab="",bty="n",main=HTML("Distribution théorique"),type = "l", xlim = c(x.lim.inf, x.lim.sup), cex.main = cex.main.title)  
 }
...
if(is.null(cv$samples.x.mat)){
  ...
  X <-c()
  par(mai=c(0.5,0.5,0.5,0.5))
  plot(X, Y, main=HTML("Histogramme des données d'échantillonnage"), xlim = c(x.lim.inf, x.lim.sup), ylim = c(0,10), xlab ="", bty="n", ylab = "", cex.main = cex.title) 
 }
else{
  par(mai=c(0.5,0.5,0.5,0.5))
  hist(cv$samples.x.mat, freq = TRUE, xlab = "",breaks = 50, col = 'grey',main = HTML("Histogramme des données d'échantillonnage"), xlim = c(x.lim.inf, x.lim.sup), cex.main = cex.title)
}        
